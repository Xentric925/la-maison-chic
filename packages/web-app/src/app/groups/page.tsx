...
const GroupsPage = () => {
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR(`${API_URL}/groups?page=${currentPage - 1}&limit=10`, fetcher, {
  ...
  if (error)
    return (
      <div className='text-center text-red-500'>
        <p>Error fetching groups. Please try again later.</p>
      </div>
    );
  ...
  return (
    <>
      ...
      <div className='flex flex-col h-full items-center justify-between'>
        ...
        <PaginationComponent
          page={currentPage}
          totalPages={groups?.next ? currentPage + 1 : currentPage}
          baseUrl={`/groups`}
        />
      </div>
    </>
